...
✅ **Auto-scan repositories** จาก path ที่กำหนด (ใหม่!)  
...
# 5. หรือใช้ scan_paths ให้ระบบค้นหา repos อัตโนมัติ
...
TalkBut สามารถค้นหา git repositories อัตโนมัติจาก path ที่กำหนดใน config:
...
git:
  ...
  repositories:
    - path: /Users/yourname/project-a
      name: "project-a"
  
  # วิธีที่ 2: Auto-scan หา repos ใน path
  scan_paths:
    - /Users/yourname/Documents/GitHub
    - /Users/yourname/projects
  scan_depth: 2  # ความลึกในการค้นหา
```

### scan_depth คืออะไร?

`scan_depth` คือความลึกของ folder ที่จะค้นหา git repos:

```
/Users/yourname/                    ← depth 0 (เริ่มต้น)
├── project-a/.git ✅               ← depth 1 (เจอ!)
├── Documents/                      ← depth 1
│   ├── GitHub/                     ← depth 2
│   │   ├── repo-1/.git ✅          ← depth 2 (เจอ!)
│   │   └── repo-2/.git ✅          ← depth 2 (เจอ!)
│   └── work/                       ← depth 2
│       └── secret-project/.git ❌  ← depth 3 (ไม่เจอถ้า depth=2)
```
...
ระบบจะรวม repos จากทั้ง `repositories` และ `scan_paths`:
- Repos ที่ระบุตรงๆ ใน `repositories` จะมี priority สูงกว่า
- Repos ที่ซ้ำกัน (path เดียวกัน) จะถูก deduplicate อัตโนมัติ
